...
for name in result:
    G.add_node(name)
```
* У каждого ключа обходим массив значений, создавая для каждого элемента узел и добавляя пару -- ключ и элемент _ массива _ значений этого ключа -- в массив с рёбрами edge:
```py
